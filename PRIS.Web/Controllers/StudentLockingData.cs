...
{
    public class StudentLockingData
    {
        public static StudentLockDataListViewModel PrepareData(string examId, string courseId, string cityId, string searchString, string sortOrder, ref List<Student> students)
        {
            ...
            var model = CourseMappings.StudentLockDataListToViewModel(invitationToStudy);
        ...
        private static List<StudentLockDataViewModel> CreateViewModelList(List<Student> students)
        {
            var invitationToStudy = new List<StudentLockDataViewModel>();
            students.ForEach(x => invitationToStudy
                .Add(CourseMappings.StudentLockDataToViewModel(x, x.ConversationResult, x.StudentCourses, x.Result)));
            return invitationToStudy;
        }

        private static void AddSelectedCourseToModel(string courseId, List<SelectListItem> stringCourses, StudentLockDataListViewModel model)
        {
        ...
        private static void AddSelectedCityToModel(string cityId, List<SelectListItem> stringCities, StudentLockDataListViewModel model)
        {
        ...
        private static void AddSelectedAcceptancePeriodToModel(string examId, List<SelectListItem> stringAcceptancePeriods, StudentLockDataListViewModel model)
        {
        ...
        private static void SortStudents(string sortOrder, List<StudentLockDataViewModel> invitationToStudy, StudentLockDataListViewModel model)
        {
            var sort = new Dictionary<string, List<StudentLockDataViewModel>>
            {
            ...
            if (sortOrder != null)
            {
                if (sort.ContainsKey(sortOrder))
                    model.StudentDataLocking = sort[sortOrder];
            }
        }

        private static List<StudentLockDataViewModel> SearchForStudents(string searchString, List<StudentLockDataViewModel> invitationToStudy)
        {
        ...
        private static List<SelectListItem> DropdownForCities(List<Student> students)
        {
            ...
            stringCities.Add(new SelectListItem { Value = "", Text = "" });
        ...
        private static List<SelectListItem> DropdownForExams(List<Student> students)
        {
            ...
            stringAcceptancePeriods.Add(new SelectListItem { Value = "", Text = "" });
